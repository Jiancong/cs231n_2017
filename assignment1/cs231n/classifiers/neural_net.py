...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # W1: (D, H), X:(N, D), W2:(H, C)
    # FC1 layer.
    fc1_activation = np.dot(X, W1) + b1 # (N, H)  = (N, D) * (D, H) + (H)
   
    # Relu layer.
    # relu_1_activation : (N, H)
    relu_1_activation = fc1_activation # (N, H)
    relu_1_activation[relu_1_activation < 0] = 0

    # FC2 layer.
    # fc2_activation : (N, C)
    fc2_activation = np.dot(relu_1_activation, W2) + b2 # (N, C) = (N, H) * (H, C) + (C)
    ...
    #############################################################################
    # Calculate dSoftmax - the gradient wrt softmax scores.
    dSoftmax = softmax_scores  # (N, C)
    dSoftmax[range(N),y] = dSoftmax[range(N),y] - 1 # This is a magic 

    # Backprop dSoftmax to calculate dW2 and add regularisation derivative.
    dW2 = np.dot(relu_1_activation.T, dSoftmax) # (H,C): (H, N) * (N, C)
    dW2 /= N
    dW2 += 2*reg*W2
    grads['W2'] = dW2
   
    # Backprop dSoftmax to calculate db2.
    db2 = dSoftmax/N * 1 # (N, C) 
    ...
    dW1 = np.dot(X.T, dRelu1) # (D, H) = (N, D).T * (N, H)
    dW1 /= N
    dW1 += 2*reg*W1
    grads['W1'] = dW1
  ...
  def predict(self, X):
    ...
    y_pred = np.argmax(pred_scores, axis=1) #(N, 1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
